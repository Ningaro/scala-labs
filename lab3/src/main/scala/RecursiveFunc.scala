...
def TestRucursiveFunc(): Unit = {
  ...
  println(
    "testReverse(cons) = "
    +
  RecursiveFunctions.testReverse(cons)
  );
  println(
    "testMap(cons, (num: Int) => num * 2) = "
    +
  ...
  println(
    "testFlatMap(cons, (x: Int) => Cons(x * 2, Nil())) = "
    +
    RecursiveFunctions.testFlatMap(cons, (x: Int) => Cons(x * 2, Nil()))
  );
}